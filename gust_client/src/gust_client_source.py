...
class Gust_Client:
    ...
    def Recieve_File(Socket, Source_Name):
    ...
    def Transfer_Source(Socket, Target_Source):
        ...
        Gust_Client.Recieve_File(Socket, source_name)
    ...
    @Run_Client
    @Connect_Session
    def Transfer_All(Socket):

        Socket.send(Commands_Global.COMMANDS["transfer all"]["command"].encode())
        encrypted_message = Gust_Client.Encrypt_Session(Commands_Global.COMMANDS["transfer all"]["command"])
        if not Gust_Client.Send_Request(Socket, encrypted_message):
            print("Connection Issue occured")
            Gust_Client.Close_Socket(Socket)
            return False
